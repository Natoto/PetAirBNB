...
-(NSInteger)indexOfImage:(NSString *)url
{
    ...
    for (Index = 0 ;Index < self.imgURLarray.count;Index++) {
        NSString *acontent =[self.imgURLarray objectAtIndex:Index];
        if ([acontent isEqualToString:url]) {
            break;
        }
    }
    return Index;
...
-(IBAction)menuButtonTap:(id)sender
{
    if (self.actionSheet) {
        ...
        float imgsize =[[ToolsFunc sharedInstance] imageSizewithUrl:url];
...
-(void)loadSubViews:(NSArray *)urls imgindex:(NSInteger)imgindex
{
    ...
    {
        MRZoomScrollView  *_zoomScrollView = (MRZoomScrollView *)[self.scrollView viewWithTag:TAG_ZOOMSCRVIEWSTART + i];
        if (!_zoomScrollView) {
        ...
        if (!_zoomScrollView.imageView.image) {
            [_zoomScrollView.imageView GET:imgurl useCache:YES];
        }
...
-(id)initWithFrame:(CGRect)frame withurl:(NSString *)url target:(id)target andSEL:(SEL)selaction contentAry:(NSArray *)contentAry
{
    ...
    if (self) {
        ...
        if (self.contentAry.count) {
            self.imgURLarray=[NSMutableArray arrayWithArray:self.contentAry];
        }
        else
        {
...
//            content *acontent =[self.imgURLarray objectAtIndex:i];
            NSString *originimg =[self.imgURLarray objectAtIndex:i];
            [_zoomScrollView.imageView GET:originimg useCache:YES];
